...
class CTextFilter {
	...
	public static function doFilter($text, $filter) {
		...
		foreach($filters as $func) {
			if(isset($valid[$func])) {
				$text = call_user_func_array(array('self', $valid[$func]), array($text));
			}
			else {
	...
	private static function markdown($text) {
	  require_once(__DIR__ . '/../../public_html/php-markdown/Michelf/Markdown.php');
	  require_once(__DIR__ . '/../../public_html/php-markdown/Michelf/MarkdownExtra.php');
	  return \Michelf\MarkdownExtra::defaultTransform($text);
	}
	
	// hur detta fungerar undrar jag.. Iom att det redan finns en sån funktion i PHP.. 
	private static function nl2br($text) {
		return nl2br($text);
	}
}